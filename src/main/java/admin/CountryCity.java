...
public class CountryCity extends BaseInit {
	...
	public void countryCity() throws InterruptedException, IOException {
		...
		if (ImageFile.isDisplayed()) {
			...
			wait.until(ExpectedConditions.elementToBeClickable(isElementPresent("GCCBack_id")));
			...
			if (ImageFile.isDisplayed()) {
				...
				// SelectRecord
				WebElement SearcheItem = isElementPresent("GCCZSSelectItem_xpath");
				highLight(SearcheItem, driver);
				js.executeScript("arguments[0].click();", SearcheItem);
				System.out.println("Selected the zipcode");
				logs.info("Selected the zipcode");
				Thread.sleep(2000);

				wait.until(ExpectedConditions.invisibilityOfElementLocated(By
						.xpath("//*[@class=\"dx-overlay-content dx-popup-normal dx-popup-draggable dx-resizable\"]")));

				// AirportID
				wait.until(
						ExpectedConditions.visibilityOfElementLocated(By.xpath("//*[@formcontrolname=\"airportId\"]")));
				wait.until(ExpectedConditions.elementToBeClickable(isElementPresent("GCCAirport_xpath")));
