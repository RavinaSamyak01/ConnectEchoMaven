...
public class ServiceRealRate extends BaseInit {
	@Test
	public void serviceRealRate() throws InterruptedException, IOException {
		System.out.println("--------------Testing ServiceRealRate---------------");
		logs.info("--------------Testing ServiceRealRate---------------");
		...
		for (int count = 0; count < RateItem.size(); count++) {
			...
			if (headervalue.equalsIgnoreCase(" Service Real Rate")) {
				WebElement Hiteam = RateItem.get(count);
				highLight(Hiteam, driver);
				act.moveToElement(Hiteam).click().perform();
				System.out.println("Clicked on ServiceRealRate");
				logs.info("Clicked on ServiceRealRate");
				waitForPageLoad();
				...
				Thread.sleep(1000);
				getScreenshot("ServiceRealRate", "ServiceRealRate", driver);

				// Create-Enable or disabled
				boolean Create = isElementPresent("SRRCreate_id").isEnabled();
				if (Create) {
				...
				// Service
				highLight(isElementPresent("SRRService_id"), driver);
				isElementPresent("SRRService_id").click();
				Thread.sleep(1000);
				isElementPresent("SRRService_id").sendKeys("Tser10 (AutomationTestService)");
				System.out.println("Entered Service Name");
				logs.info("Entered Service Name");
				Thread.sleep(1000);
				isElementPresent("SRRService_id").sendKeys(Keys.ENTER);
				System.out.println("Selected service from the dropdown");
				logs.info("Selected service from the dropdown");
				isElementPresent("SRRService_id").sendKeys(Keys.TAB);

				// Create-Enable or disabled
				Create = isElementPresent("SRRCreate_id").isEnabled();
				if (Create) {
					System.out.println("Create button is Enabled");
					logs.info("Create button is Enabled");
				...
				getScreenshot("GridAfterSearch", "ServiceRealRate", driver);
				...
				System.out.println("Records in the grid are==" + isElementPresent("GridRecord_xpath").getText());
				logs.info("Records in the grid are==" + isElementPresent("GridRecord_xpath").getText());
				getScreenshot("GridAfterClear", "ServiceRealRate", driver);
				...
				System.out.println("Entered Category Name");
				logs.info("Entered Category Name");
				highLight(isElementPresent("APSearch_id"), driver);
				isElementPresent("APSearch_id").sendKeys(Keys.ENTER);
				...
				if (ImageFile.isDisplayed()) {
					...
					waitForPageLoad();
					Thread.sleep(2000);
					getScreenshot("Editor_", "ServiceRealRate", driver);
				...
				if (ImageFile.isDisplayed()) {
					...
					getScreenshot("ServiceRealRateDelete_", "ServiceRealRate", driver);
					...
					getScreenshot("ServiceRealRateKeepIT_", "ServiceRealRate", driver);
					wait.until(ExpectedConditions
							.visibilityOfAllElementsLocatedBy(By.xpath("//div[@aria-label=\"Data grid\"]")));
					wait.until(ExpectedConditions
							.invisibilityOfElementLocated(By.xpath("//*[@class=\"dx-loadpanel-content-wrapper\"]")));
					...
					getScreenshot("ServiceRealRateDeleteIT_", "ServiceRealRate", driver);
					wait.until(ExpectedConditions
							.visibilityOfAllElementsLocatedBy(By.xpath("//div[@aria-label=\"Data grid\"]")));
					wait.until(ExpectedConditions
							.invisibilityOfElementLocated(By.xpath("//*[@class=\"dx-loadpanel-content-wrapper\"]")));
				...
				// PageSize
				pageSize("ServiceRealRate");

				// Pagination
				pagination("ServiceRealRate");

				// NormalView
				normalView("ServiceRealRate");

				// Columns
				columns("ServiceRealRate");

				break;
			} else {
				System.out.println("ServiceRealRate is not exist in Admin menu");
				logs.info("ServiceRealRate is not exist in Admin menu");
			}
		}
